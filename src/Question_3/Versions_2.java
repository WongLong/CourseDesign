...
public class Versions_2{
	public static void start(){
		...
		final RadioButton radioBt = new RadioButton("随机创建");
		...
		bt1.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				...
				TailModel model = new TailModel(cp.orderNum, false);
				...
				if (path.size() == 1) {
					...
					for (int i = 1; i < path.size(); i++) {
						...
						ArrayList<Integer> different = getDifferentIndex(coin, last);
						CoinPane newCP = new CoinPane(coin, cp.orderNum, different);
